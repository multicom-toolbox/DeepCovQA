...
foreach $item (@seqarray)
{
	...
	print "perl $GLOBAL_PATH/scripts/encoding_ss.pl $fasta_file1 $ss_output1 \n";
	$status = system("perl $GLOBAL_PATH/scripts/encoding_ss.pl $fasta_file1 $ss_output1 ");
	if($status)
	{
		die "Failed to run run_SCRATCH-1D_predictors.sh \n";
	}
	...
	print "perl $GLOBAL_PATH/scripts/encoding_acc.pl $fasta_file1 $sa_output1 \n";
	$status = system("perl $GLOBAL_PATH/scripts/encoding_acc.pl $fasta_file1 $sa_output1 ");
	if($status)
	{
	...
	print "perl $GLOBAL_PATH/scripts/gen_encoding.pl $fasta_file1 $new_dir \n";
	$status = system("perl $GLOBAL_PATH/scripts/gen_encoding.pl $fasta_file1 $new_dir");
	if($status)
	{
